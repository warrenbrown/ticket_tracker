...
RSpec.feature 'Users can receive notifications about tickets' do
  ...
  let(:bob) { FactoryGirl.create(:user, email: 'bob@examble.com') }
  ...
  scenario 'ticket authors automatically receive notifications' do
    ...
    expected_subject = "[ticket tracker] #{project.name} - #{ticket.name}"
  ...
  scenario 'comment authors are automatically subscribed to a ticket' do
    fill_in 'Text', with: 'Is it done yet.'
    click_button 'Create Comment'
    click_link 'Sign out'
    ...
    login_as(alice)
    visit project_ticket_path(project, ticket)
    fill_in 'Text', with: 'No it is not sorry.'
    click_button 'Create Comment'
